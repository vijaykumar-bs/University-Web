            ...
            <div class="collapse navbar-collapse" id="mainNav">
                <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
                    ...
                    <li class="nav-item dropdown dropdown-mega">
                        <a class="nav-link dropdown-toggle" href="academics.html" data-bs-toggle="dropdown">Academics</a>
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="admissions.html" data-bs-toggle="dropdown">Admissions</a>
                    ...
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li><a class="dropdown-item" href="student-services.html">Student Services</a></li>
                        </ul>
                    </li>
                    ...
                    <li class="nav-item"><a class="nav-link" href="news.html">News & Blogs</a></li>
                </ul>
...
fs.readdirSync(directoryPath).forEach(file => {
    if (path.extname(file) === '.html' && file !== 'index.html') {
        ...
        // Matches any <nav class="navbar...">...</nav> block
        const navRegex = /<nav[^>]*class="navbar[^>]*">([\s\S]*?)<\/nav>/i;
        if (navRegex.test(content)) {
            content = content.replace(navRegex, STANDARD_NAVBAR);
        }
        ...
        // 3. Standardize Contact Bar
        const contactBarRegex = /<!-- Floating Contact Bar -->[\s\S]*?<\/div>/i;
        if (contactBarRegex.test(content)) {
            content = content.replace(contactBarRegex, STANDARD_CONTACT_BAR);
        } else {
            // If not found, insert after <body> tag
            content = content.replace(/<body[^>]*>/i, (match) => match + STANDARD_CONTACT_BAR);
        }

        // 4. Ensure Scripts are at the end
        if (!content.includes('assets/js/ai-chatbot.js')) {
            content = content.replace(/<\/body>/i, '    <script src="assets/js/ai-chatbot.js"></script>\n</body>');
        }
